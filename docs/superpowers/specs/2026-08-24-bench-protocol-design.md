...
**決定（2026-08-24 修正）：上行用 POST，下行用 Durable Object + WebSocket。不用 Queue。**

> **修正紀錄**：本節初版寫「不用 WebSocket」，論證是「MCP 是 pull，即時通道沒有客戶」。
> 那個論證**只對了一半**——它成立於「Claude 被動觀察」，但不成立於
> **「使用者對話中，Claude 伸手改頁面」**。春春指出後修正。
...
- ❌ 量完自動有人跟你講話 → 做不到（需要常駐 agent，見 §3.5）
- ✅ **你跟 Claude 講話的當下，Claude 改頁面** → 完全做得到

第二條才是價值所在，而且**只有即時通道做得到**：

| 使用者說（手抓表筆、對手機） | 有 WebSocket | 沒有 |
|---|---|---|
| 「pin 7 標跳過，我構不到」 | 頁面立刻打勾 | 放下表筆走過去按 |
| 「清單改成只量 1–8 腳」 | 立刻換 | 走過去重取任務 |
| **「下一腳」** | **立刻跳** | **手必須離開表筆** |
| 「基準換成 MMS-MB-0007」 | 立刻重算 | 重取一次 |

「下一腳」用講的推進，兩手全程不離表筆——外設也做不到，這是即時通道獨有的。

觸發點都是**使用者正在對 Claude 講話**，也就是 Claude 正好活著的時刻。所以 pull 不構成障礙：Claude 不需要主動醒來，只需要在被叫醒時**有一條線能碰到頁面**。

### 上行為什麼仍然是 POST
量完上傳是一跳就完的事，不需要連線保證。POST 失敗就重試，語意單純。把上行也塞進 WebSocket 只會讓「連線斷了資料在哪」變複雜。
...
### DO 與 D1 的分工
**DO 管連線，D1 管資料。** 一個工作台一個 DO（key 用維修單號或固定的單人 key），只負責：
- 持有瀏覽器的 WebSocket
- 接收 MCP 那端的指令（`goto_pin` / `set_pin` / `replace_task` / `set_baseline`）並轉發給頁面
- 不存業務資料——那是 D1 的事

**必須用 WebSocket Hibernation API**，否則閒置連線會一直計 duration 費用。Hibernation 下客戶端保持連線但 DO 可以休眠，有訊息才醒。

### 連線斷掉怎麼辦
頁面必須顯示連線狀態（已連線／重連中／離線）。**離線時所有功能照常可用**——量測、編輯、上傳全部不依賴 WebSocket，只是 Claude 暫時碰不到頁面。這是刻意的：連線是加分，不是前提。
...
**但反過來可以**：你在對話中叫我改頁面，透過 DO 立刻生效（見 §3）。所以「交棒」只適用於**你要我判讀結果**這個方向。
...
| **DO 即時通道**（WS＋Hibernation＋重連＋連線狀態 UI） | **半天～1 天** |
...
| ~~WebSocket 即時推送~~ | **已推翻（2026-08-24）**。原論證「MCP 是 pull 所以沒客戶」只對了一半：它成立於被動觀察，不成立於「對話中主動改頁面」。後者才是價值所在，見 §3 |
