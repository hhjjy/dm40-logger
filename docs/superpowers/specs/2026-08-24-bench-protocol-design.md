...
### 介面（待確認，見 §11）
建議 **量測頁＝腳位圖 ＋ 大讀值**（原型版本 D）、**回顧頁＝表格**（版本 B），Tab 切換——與工具現有的「量測中 ↔ 回顧」結構一致。
...
1. **介面版本** — 建議量測頁用原型的 D、回顧頁用 B。春春針對 D 的元件（腳位圖間距、pin 1、停留秒數）給過詳細回饋，但未明確選定。**若不同意，這是唯一需要改的決定。**
2. **匯入腳數不一致** — 目前匯入 SVG 只有圖跟著變，資料列數沒調整。正式版應以 SVG 的 `data-pin` 為準重建腳位清單。已記在 `prototype/README.md`。
